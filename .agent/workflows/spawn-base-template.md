...
3. For each track, define:
   ...
   - **Output format** (structured findings per workflow-specific template)
...
3. **Spawn workers** (or persona-switch) per the chosen strategy.
   - CLI: Use `spawn-agent` skill.
   - Persona-switch: Assume persona → investigate → write report → switch back.
...
2. **Validate completeness** per workflow-specific checklist.
3. Normalize reports into a consistent format for cross-review.

**Output:** Validated reports in `.hc/<domain>/YYYY-MM-DD-[topic]-track-N.md`
...
4. **Surface conflicts/correlations** in a structured log.
