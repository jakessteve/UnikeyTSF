...
- When wearing the **@dev hat**: Write feature code. This is expected and required.
...
- MUST delegate code changes to coding personas (@dev, @qc, @designer, @devops).

### The Key Rule
> Don't plan and code **in the same mental step**. Decide what to do as @pm, then switch persona to do it.

If a planning persona needs to modify source code → switch to @dev persona first. This ensures the coding agent's rules (type safety, testing, DRY) are active during implementation.
...
```
Plan approved → Mandatory Spawn Gate (§3.1.8) → /delegate-task → @dev worker executes → @pm reviews
```

**The only exception:** Changes to 1-3 files can use persona-switching (switch to @dev hat explicitly before touching code). But even then, @pm MUST announce the persona switch.
...
Before @dev persona touches ANY source files (`.ts`, `.tsx`, `.css`, `.rs`, `.json`):
