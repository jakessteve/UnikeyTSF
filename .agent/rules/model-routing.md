...
## 9. Fallback Setup (Quota Exhaustion)

If the Gemini CLI or primary model hits rate limits or quota exhaustion:
1. **Active Core Work:** Auto-pivot the worker model flag from `GEMINI-H/Plan` to `SONNET/Fast` (or vice-versa), or to a configured local/secondary API key.
2. **Background Swarms:** Pause parallel execution and queue remaining tracks to avoid recursive failure. Escalate to User.
3. **Orchestrator Level:** If the PM context itself hits token ceilings, immediately trigger the `Anti-Context-Exhaustion (Circuit Breaker)` defined in `anti-patterns-core.md` section 4.
