...
### Step 4 — DRY Check
Consult `code-standards.md`. `grep_search` for existing services, utilities, or middleware that overlap.
...
### Step 6 — Type Safety End-to-End
Consult `code-standards.md`. Verify type flow with zero `any`.
...
### Step 8 — Self-Review
Consult `code-standards.md` and trigger `code-review` checks before proceeding.
...
### Step 11 — Anti-Hallucination Check
Consult `anti-patterns-core.md` Section 1. All paths, endpoints, and method calls MUST be verified before proceeding.
